...
namespace AlphaTest.Infrastructure.Database.QueryExtensions
{
    public static class TestQueryExtensions
    {
        ...
        {
            return title is null ? query : query.Where(test => EF.Functions.Like(test.Title, "%" + title + "%"));
        }

        public static IQueryable<Test> FilterByTopic(this IQueryable<Test> query, string topic)
        {
            return topic is null ? query : query.Where(test => EF.Functions.Like(test.Topic, "%" + topic + "%"));
        }
            ...
            Guid? authorOrContributorID)
        {
            return authorOrContributorID is null
                ...
                : query.Where(test => 
                    ...
                    EF.Property<List<Contribution>>(test, "_contributions").Any(contribution =>
